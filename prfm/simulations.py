...
def add_PSFR_model_lines(Wmin=2,Wmax=8):
    for y,ls in zip([1.e3,'tigress-classic','tigress-classic-decomp'],['-',':','--']):
        add_PSFR_model_line(Wmin=Wmin,Wmax=Wmax,model=y,
                            ls=ls,color='k',label=r'$\Upsilon:$' + '{}'.format(y))
...
def add_yield_model_lines(Wmin=2,Wmax=8,comp='th'):
    for y,ls in zip(['tigress-classic-decomp','tigress-ncr-decomp','tigress-ncr-decomp-Z01'],[':','--','-']):
...
def load_sim_data():
    ...
    data.dPimag = np.array([np.nan,np.nan,0.94,1.04,1.03,1.10,1.16])*1.e3
    data.oPimag = np.array([np.nan,np.nan,0.46,0.73,0.87,0.91,1.83])*1.e3

    data.SFR_std = np.array([0.38,0.28,0.16,0.16,0.15,0.16,0.14])*1.e-3
    data.Pturb_std = np.array([3.05,2.61,2.42,2.66,1.46,2.11,1.12])*1.e3
    data.Pth_std = np.array([0.40,0.29,0.18,0.20,0.18,0.16,0.16])*1.e3
    data.dPimag_std = np.array([np.nan,np.nan,0.21,0.18,0.17,0.22,0.18])*1.e3
    data.oPimag_std = np.array([np.nan,np.nan,0.15,0.18,0.23,0.29,0.45])*1.e3
    ...
    # my re caclulation
    data.SFR = np.array([ 9.55e-01, 6.18e-01, 7.81e-02, 7.39e-02, 3.60e-03, 3.11e-03, 6.69e-05])
    data.Pturb = np.array([ 1.74e+06, 6.16e+05, 3.09e+05, 1.68e+05, 6.01e+03, 5.15e+03, 1.42e+02])
    data.Pth = np.array([ 1.09e+05, 7.00e+04, 1.63e+04, 1.35e+04, 4.92e+03, 2.32e+03, 2.58e+02])
    data.oPimag = np.array([ 1.44e+05, 1.24e+05, 8.77e+03, 7.55e+03, 4.37e+03, 7.99e+02, 1.88e+02])
    data.dPimag = np.array([ 3.17e+05, 1.65e+05, 1.14e+04, 1.06e+04, 3.42e+03, 1.13e+03, 7.34e+01])

    data.SFR_std = np.array([ 2.76e-01, 9.17e-02, 1.06e-02, 2.59e-02, 1.47e-03, 2.14e-03, 7.04e-05])
    data.Pturb_std = np.array([ 1.57e+06, 2.47e+05, 3.83e+05, 1.43e+05, 2.23e+03, 2.77e+03, 1.09e+02])
    data.Pth_std = np.array([ 6.85e+04, 1.54e+04, 9.60e+03, 3.80e+03, 1.44e+03, 1.30e+03, 1.73e+02])
    data.oPimag_std = np.array([ 1.60e+05, 7.36e+04, 1.21e+04, 6.88e+03, 1.25e+03, 8.13e+02, 2.08e+02])
    data.dPimag_std = np.array([ 2.51e+05, 7.71e+04, 1.37e+04, 1.09e+04, 1.41e+03, 7.40e+02, 3.97e+01])
    data.convert_linear_log()
    data.get_Ptotal()
    data.get_yield()
    ...
    data.Pturb = np.array([0.137,0.512,1.49,4.49,1.44,4.26,12.7,57.4])*1.e6
    data.Pth = np.array([0.128,0.424,0.966,1.99,1.01,2.32,3.82,9.51])*1.e6

    data.SFR_std = np.array([0.027,0.099,0.37,0.86,0.24,0.98,2.5,7.4])
    data.Pturb_std = np.array([0.0022,0.079,0.19,0.78,0.33,0.82,2.7,19.7])*1.e6
    data.Pth_std = np.array([0.0022,0.051,0.106,0.32,0.30,0.42,0.78,2.06])*1.e6
    data.convert_linear_log()
    ...
    data = PRFM_NCR_Z1
    data.SFR = np.array([ 1.14e-02, 2.60e-03, 2.97e-03, 1.13e-04, 1.27e-01, 8.60e-02, 2.78e-02, 3.43e-02, 2.47e-01, 1.33e-01, 2.78e-01])
    data.Pturb = np.array([ 1.86e+04, 5.58e+03, 8.35e+03, 5.87e+02, 1.96e+05, 2.78e+05, 4.23e+04, 5.00e+04, 1.59e+05, 8.78e+04, 9.87e+05])
    data.Pth = np.array([ 1.07e+04, 4.02e+03, 4.39e+03, 4.00e+02, 5.70e+04, 3.93e+04, 2.00e+04, 2.43e+04, 8.81e+04, 3.82e+04, 1.06e+05])
    data.oPimag = np.array([ 8.09e+03, 4.74e+03, 4.63e+03, 3.02e+02, 4.23e+04, 1.93e+04, 2.48e+04, 2.45e+04, 3.32e+05, 3.86e+05, 5.66e+03])
    data.dPimag = np.array([ 9.48e+03, 3.33e+03, 3.22e+03, 1.20e+02, 3.20e+04, 1.74e+04, 1.34e+04, 1.86e+04, 1.43e+05, 6.21e+04, 2.48e+04])
    data.SFR_std = np.array([ 3.05e-03, 7.88e-04, 1.20e-03, 6.11e-05, 6.07e-02, 4.91e-02, 8.16e-03, 6.11e-03, 7.50e-02, 5.23e-02, 1.15e-01])
    data.Pturb_std = np.array([ 6.63e+03, 2.09e+03, 5.22e+03, 4.73e+02, 1.38e+05, 2.62e+05, 2.38e+04, 2.54e+04, 7.14e+04, 4.55e+04, 4.78e+05])
    data.Pth_std = np.array([ 3.15e+03, 1.10e+03, 1.61e+03, 2.08e+02, 3.25e+04, 2.23e+04, 6.36e+03, 6.32e+03, 3.46e+04, 1.15e+04, 6.88e+04])
    data.oPimag_std = np.array([ 4.42e+03, 2.27e+03, 2.21e+03, 2.36e+02, 5.46e+04, 2.51e+04, 2.27e+04, 2.37e+04, 1.04e+05, 6.99e+04, 9.28e+03])
    data.dPimag_std = np.array([ 3.61e+03, 1.22e+03, 1.46e+03, 7.17e+01, 3.72e+04, 1.95e+04, 6.32e+03, 1.04e+04, 5.93e+04, 3.79e+04, 3.26e+04])
    data.convert_linear_log()
    data.get_Ptotal()
    data.get_yield()
    ...
    data = PRFM_NCR_lowZ
    data.SFR = np.array([ 7.92e-03, 1.90e-03, 1.27e-03, 1.17e-03, 1.92e-03, 1.29e-03, 1.32e-03, 4.08e-05, 8.85e-02, 1.69e-02, 2.52e-02, 1.86e-02, 2.02e-02, 1.08e-01, 2.61e-01])
    data.Pturb = np.array([ 2.03e+04, 4.65e+03, 3.09e+03, 3.15e+03, 5.71e+03, 3.36e+03, 3.22e+03, 1.49e+02, 1.27e+05, 3.44e+04, 3.88e+04, 3.37e+04, 5.00e+04, 8.66e+04, 3.20e+05])
    data.Pth = np.array([ 2.04e+04, 5.89e+03, 6.80e+03, 5.74e+03, 5.75e+03, 6.96e+03, 6.53e+03, 9.37e+02, 8.53e+04, 3.41e+04, 3.18e+04, 3.97e+04, 3.24e+04, 1.24e+05, 1.62e+05])
    data.oPimag = np.array([ 1.18e+04, 4.87e+03, 6.35e+03, 7.95e+03, 5.35e+03, 6.12e+03, 6.05e+03, 1.77e+02, 5.13e+04, 2.96e+04, 3.82e+04, 3.83e+04, 3.18e+04, 4.14e+05, 1.08e+04])
    data.dPimag = np.array([ 9.59e+03, 3.03e+03, 2.46e+03, 2.51e+03, 3.27e+03, 2.38e+03, 2.71e+03, 1.04e+02, 5.24e+04, 1.66e+04, 1.83e+04, 2.07e+04, 1.89e+04, 9.55e+04, 3.79e+04])
    data.SFR_std = np.array([ 2.90e-03, 8.53e-04, 7.95e-04, 9.17e-04, 5.18e-04, 8.13e-04, 7.20e-04, 3.62e-05, 3.26e-02, 5.95e-03, 6.39e-03, 7.73e-03, 4.48e-03, 2.47e-02, 1.12e-01])
    data.Pturb_std = np.array([ 9.36e+03, 1.77e+03, 1.04e+03, 1.50e+03, 2.64e+03, 1.13e+03, 1.12e+03, 6.05e+01, 6.38e+04, 1.97e+04, 2.06e+04, 1.63e+04, 2.68e+04, 2.15e+04, 1.71e+05])
    data.Pth_std = np.array([ 5.11e+03, 1.72e+03, 1.74e+03, 2.12e+03, 1.14e+03, 2.04e+03, 1.39e+03, 2.95e+02, 3.26e+04, 8.52e+03, 6.56e+03, 1.31e+04, 9.48e+03, 2.10e+04, 5.55e+04])
    data.oPimag_std = np.array([ 5.44e+03, 2.36e+03, 2.25e+03, 3.32e+03, 1.39e+03, 2.38e+03, 2.51e+03, 1.22e+02, 4.10e+04, 2.03e+04, 3.42e+04, 3.70e+04, 2.33e+04, 4.80e+04, 1.10e+04])
    data.dPimag_std = np.array([ 3.49e+03, 1.29e+03, 1.05e+03, 1.56e+03, 1.51e+03, 1.16e+03, 1.28e+03, 4.13e+01, 3.23e+04, 9.10e+03, 8.52e+03, 1.40e+04, 9.68e+03, 3.06e+04, 2.57e+04])
    data.convert_linear_log()
    data.get_Ptotal()
    data.get_yield()
