...
The central idea is to build an observationally anchored simulation and inference program for the star-forming interstellar medium. PHANGS defines the distribution of real galactic environments, TIGRESS-NCR provides controlled numerical experiments in those environments, and ILI provides a practical way to infer physical parameters and quantify model uncertainty when explicit likelihoods are unavailable.

## Goals

1. Define a PHANGS-informed parameter space for PRFM/TIGRESS comparisons.
2. Build representative sampling strategies for observed PHANGS environments.
3. Generate a TIGRESS simulation suite over the sampled parameter space.
...
The project should also help extract more information from existing and future surveys. Instead of comparing a small number of hand-picked simulations to a few summary plots, the simulation suite should be designed from the start as an inference-ready dataset: broad enough to cover the observed PHANGS environments, structured enough to train emulators, and rich enough to support field-level or summary-statistic comparisons.

## Data Products

- PHANGS aperture-level tables with derived PRFM inputs.
- Sampled PHANGS environmental parameter sets.
...
This is the first CAMELS-like step: build a controlled simulation suite whose parameter design is motivated by the observed PHANGS distribution rather than by an ad hoc grid.

- The initial suite targets one or a small number of fixed `Sigma_gas` values.
- Draw plausible environment parameters from PHANGS data, including `Sigma_star`, `Omega_d`, and `H_star`.
- Convert PHANGS-derived quantities into TIGRESS-NCR simulation inputs: `Sigma_star`, `z_star`, `Omega`, and `rho_dm`.
- Preserve physically required constraints, for example `Sigma_atom + Sigma_mol = Sigma_gas`.
- Run a suite of simulations with a standard physics parameter choice, such as fixed `qshear`, `Zgas`, `Zdust`, and feedback/population-synthesis assumptions.
- Assess the resulting `Sigma_SFR`, `Sigma_atom`, and `Sigma_mol` against PHANGS distributions.
...
The design should explicitly track whether the suite is optimized for interpolation within the PHANGS distribution, extrapolation to rare environments, or parameter inference. These goals imply different sampling choices.
...
- Compute derived quantities such as `Sigma_gas`, `Omega_d`, and `H_star`.
...
- Compare observed-pixel LHS sampling with KDE-based synthetic sampling.
- Enforce physical constraints such as `Sigma_atom + Sigma_mol = Sigma_gas`.
- Validate sampling fairness in primary fields and SFR tracers.
...
- Compare observed PHANGS distributions, sampled inputs, and model outputs.
...
- Which SFR tracer should define the primary comparison target?
- Should sampling be conditioned on galaxy/radius in addition to `Sigma_gas`?
...
- What level of sampling fairness is scientifically sufficient: 0.1, 0.15, or 0.2 dex?
