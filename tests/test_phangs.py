...
HAS_DATA = DATA_DIR.exists() and any(DATA_DIR.glob("*_annulus_0p5kpc.ecsv"))
...
class TestComputePRFMInputs:
    def test_returns_table_with_new_columns(self, synthetic_table):
        ...
        for col in ("Sigma_gas", "Omega", "qshear", "H_star"):
...
class TestRunPRFM:
    ...
    def test_omega_d_scaling_requires_selected_column(self, synthetic_table):
        with pytest.raises(ValueError, match="omega_d_col"):
            phangs.run_prfm(synthetic_table, variation={"Omega_d": 2.0})
